...
@section('title', 'Adaugă Clasă')
@section('header', 'Adaugă Clasă Nouă')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b">
                <h3 class="text-lg font-semibold">Informații Clasă</h3>
            </div>
            
            <form action="{{ route('clase.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="an" id="an" required
                            ...
                            @for($i = 1; $i <= 12; $i++)
                                <option value="{{ $i }}" {{ old('an') == $i ? 'selected' : '' }}>{{ $i }}</option>
                            @endfor
                        </select>
                        @error('an')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <input type="text" name="profil" id="profil" value="{{ old('profil') }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="an_scolar" id="an_scolar" value="{{ old('an_scolar', date('Y')) }}" required
                ...
                <div class="flex justify-end space-x-3 mt-6 pt-6 border-t">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                        <i class="fas fa-save mr-2"></i> Salvează Clasa
                    </button>
                </div>
            </form>
        </div>
