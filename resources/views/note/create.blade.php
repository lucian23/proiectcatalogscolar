...
@section('title', 'Adaugă Notă')
@section('header', 'Adaugă Notă Nouă')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b">
                <h3 class="text-lg font-semibold">Informații Notă</h3>
            </div>
            ...
            <form action="{{ route('note.store') }}" method="POST" class="p-6">
                    ...
                    <div class="col-span-2">
                        ...
                        <select name="elev_id" id="elev_id" required
                            ...
                            @foreach($elevi as $elev)
                                <option value="{{ $elev->id }}" {{ old('elev_id') == $elev->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="materie_id" id="materie_id" required
                            ...
                            @foreach($materii as $materie)
                                <option value="{{ $materie->id }}" {{ old('materie_id') == $materie->id ? 'selected' : '' }}>
                                    {{ $materie->nume }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-3 gap-6 mt-6">
                    <div>
                        ...
                        <input type="number" name="nota" id="nota" value="{{ old('nota') }}" required min="1" max="10" step="0.01"
                    ...
                    <div>
                        ...
                        <select name="tip" id="tip" required
                            ...
                            <option value="curenta" {{ old('tip') == 'curenta' ? 'selected' : '' }}>Curentă</option>
                            <option value="teza" {{ old('tip') == 'teza' ? 'selected' : '' }}>Teză</option>
                            <option value="recuperare" {{ old('tip') == 'recuperare' ? 'selected' : '' }}>Recuperare</option>
                        </select>
                        @error('tip')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <textarea name="observatii" id="observatii" rows="3"
                        ...
                        placeholder="Observații opționale...">{{ old('observatii') }}</textarea>
                    @error('observatii')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-3 mt-6 pt-6 border-t">
                    ...
                    <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">
                        <i class="fas fa-save mr-2"></i> Salvează Nota
                    </button>
                </div>
            </form>
        </div>
