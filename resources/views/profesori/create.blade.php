...
@section('title', 'Adaugă Profesor')
@section('header', 'Adaugă Profesor Nou')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b">
                <h3 class="text-lg font-semibold">Date Profesor</h3>
            </div>
            
            <form action="{{ route('profesori.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <select name="grad_didactic" id="grad_didactic"
                            ...
                            <option value="I" {{ old('grad_didactic') == 'I' ? 'selected' : '' }}>Gradul I</option>
                            <option value="II" {{ old('grad_didactic') == 'II' ? 'selected' : '' }}>Gradul II</option>
                            <option value="Definitivat" {{ old('grad_didactic') == 'Definitivat' ? 'selected' : '' }}>Definitivat</option>
                            <option value="Începător" {{ old('grad_didactic') == 'Începător' ? 'selected' : '' }}>Începător</option>
                        </select>
                        @error('grad_didactic')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        ...
                        @foreach($materii as $materie)
                            <label class="flex items-center">
                                <input type="checkbox" name="materii[]" value="{{ $materie->id }}" 
                                    {{ in_array($materie->id, old('materii', [])) ? 'checked' : '' }}
                ...
                <div class="flex justify-end space-x-3 mt-6 pt-6 border-t">
                    ...
                    <button type="submit" class="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700">
                        <i class="fas fa-save mr-2"></i> Salvează Profesor
                    </button>
                </div>
            </form>
        </div>
